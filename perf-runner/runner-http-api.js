...
class Client {
    ...
    startMeterValues() {
        ...
        // Configuration de phasage (avec valeurs par défaut)
        const vehiclePhases = this.phasingConfig?.vehicleActivePhases || 3;
        const powerPerPhase = this.phasingConfig?.powerPerPhase || 16;
        const evsePhases = this.phasingConfig?.evsePhases || 3;

        // Calculer la puissance totale selon les phases ACTIVES du véhicule
        let powerW;
        if (this.phasingConfig?.configured) {
            // Si phasage configuré, utiliser le calcul basé sur les phases actives
            powerW = vehiclePhases * powerPerPhase * voltage;
        } else {
            // Sinon, utiliser la puissance configurée normalement
            powerW = Math.max(0, (this.opts.powerKW ?? 7.4) * 1000);
        }
        ...
        this._mvTimer = setInterval(() => {
            ...
            // Ajouter les mesures standard (energy, power, etc.)
            ...
            if (mvConfig.mask.soc) {
                sampledValue.push({
                    value: String(Math.round(soc * 10) / 10),
                    measurand: "SoC",
                    unit: "Percent",
                    context: "Sample.Periodic"
                });
            ...
            if (mvConfig.mask.powerOffered) {
                // Power offered basé sur les phases de l'EVSE
                const offeredA = this.phasingConfig?.configured ?
                    (evsePhases * powerPerPhase) : mvConfig.maxA;
                const offeredW = offeredA * voltage;
                sampledValue.push({
                    value: String(Math.round(offeredW)),
            ...
            // NOUVEAU: Ajouter les courants par phase si phasage configuré
            if (this.phasingConfig?.configured) {
                // Phase L1 - toujours présente
                sampledValue.push({
                    value: String(vehiclePhases >= 1 ? powerPerPhase : 0.001),
                ...
                // Phase L2 - seulement si EVSE supporte
                if (evsePhases >= 2) {
                    sampledValue.push({
                        value: String(vehiclePhases >= 2 ? powerPerPhase : 0.001),
                        measurand: "Current.Import",
                        phase: "L2",
                        unit: "A",
                        context: "Sample.Periodic"
                ...
                // Phase L3 - seulement si EVSE supporte
                if (evsePhases >= 3) {
                    sampledValue.push({
                        value: String(vehiclePhases >= 3 ? powerPerPhase : 0.001),
                        measurand: "Current.Import",
                        phase: "L3",
                        unit: "A",
                        context: "Sample.Periodic"
                ...
                // Courant total (sans phase spécifiée)
                const totalCurrent = vehiclePhases * powerPerPhase;
                sampledValue.push({
                ...
                // Mode normal sans phasage - courant calculé depuis la puissance
            ...
            if (this.phasingConfig?.configured) {
                // Log avec info de phasage
                this.addLog(`⚡ MV: ${vehiclePhases}/${evsePhases}ph, ${powerPerPhase}A/ph, ${powerKW}kW${socStr}`);
            } else {
                // Log normal
...
class SimSession extends Client {
    constructor(apiSessionId, params) {
        ...
        // NOUVEAU: Configuration phasage
        this.phasingConfig = {
            evsePhases: 3,  // Par défaut EVSE triphasé
            vehicleActivePhases: 3,  // Par défaut VE triphasé
            powerPerPhase: 16,  // Ampères par phase
            configured: false
        };
    }
}
...
app.post("/api/simu/:id/phasing", (req, res) => {
    const s = SESSIONS_MAP.get(req.params.id);
    if (!s) return res.status(404).json({ error: "Session not found" });
    ...
    // Mettre à jour la configuration de phasage
    s.phasingConfig = {
        evsePhases: Math.min(3, Math.max(1, evsePhases)),
        vehicleActivePhases: Math.min(evsePhases, Math.max(1, vehicleActivePhases)),
        powerPerPhase: Math.min(63, Math.max(6, powerPerPhase)),
        configured: true
    };

    // Calculer la puissance totale basée sur les phases ACTIVES du véhicule
    const voltage = s.opts.voltageV || 230;
    s.opts.powerKW = (s.phasingConfig.vehicleActivePhases * powerPerPhase * voltage) / 1000;

    s.addLog(` Phasage: EVSE ${evsePhases}ph, VE ${vehicleActivePhases}ph, ${powerPerPhase}A/ph`);
    s.addLog(`Puissance calculée: ${s.opts.powerKW.toFixed(1)}kW`);

    // Si une transaction est en cours, redémarrer les MeterValues
    if (s.status === "started" && s.txId && s.opts.mvEverySec > 0) {
        s.stopMeterValues();
        s.startMeterValues();
        s.addLog(` MeterValues mis à jour avec nouveau phasage`);
    }

    res.json({
        ok: true,
        config: s.phasingConfig,
        calculatedPowerKW: s.opts.powerKW
    });
});


app.get("/api/simu/:id/phasing", (req, res) => {
    const s = SESSIONS_MAP.get(req.params.id);
    if (!s) return res.status(404).json({ error: "Session not found" });

    res.json({
        config: s.phasingConfig,
        powerKW: s.opts.powerKW
    });
});
