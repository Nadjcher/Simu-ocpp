...
export function getApiBase(): string {
    if (typeof window !== "undefined") {
        return window.localStorage.getItem("runner_api") || "http://localhost:8877";
    }
    return "http://localhost:8877";
}

export const RUNNER = getApiBase(); // Appeler la fonction pour obtenir l'URL
export const API_BASE = RUNNER; // Export alias pour compatibilité avec TnrTab.tsx
...
export const tnr = {
    ...
    save: (scenario: any) => http<{ ok: boolean; id: string }>("/api/tnr", {
        method: "POST",
        body: JSON.stringify(scenario),
    }),

    record: (scenario: any) => http<{ ok: boolean; id: string }>("/api/tnr/record", {
        method: "POST",
        body: JSON.stringify(scenario),
    }),
    ...
    remove: (id: string) => http("/api/tnr/" + id, { method: "DELETE" }),

    replay: (id: string) => http<{ ok: boolean; resultId: string }>("/api/tnr/replay/" + id, {
        method: "POST"
    }),
    ...
    // Recorder
    recorderStart: (name?: string) => http("/api/tnr/recorder/start", {
        method: "POST",
        body: name ? JSON.stringify({ name }) : "{}",
    }),

    recorderStop: (id?: string) => http("/api/tnr/recorder/stop", {
        method: "POST",
        body: id ? JSON.stringify({ id }) : "{}",
    }),

    cancelRecording: () => http("/api/tnr/recorder/cancel", {
        method: "POST"
    }),

    // Méthodes pour TNRPanel
    ...
    importScenario: (formData: FormData) => fetch(`${RUNNER}/api/tnr/import`, {
        method: "POST",
        body: formData,
    }).then(res => {
